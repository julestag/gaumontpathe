...
if ((isset($_GET['s']) && !empty($_GET['s'])) && (isset($_GET['l']) && !empty($_GET['l'])) && empty($_GET['f'])) {
    ...
    $distribmovie = $bdd->query("SELECT * FROM movie INNER JOIN distributor ON movie.id_distributor = distributor.id WHERE name LIKE '%$distribb%' AND movie.title like '%$cinemac%'");
}
?>